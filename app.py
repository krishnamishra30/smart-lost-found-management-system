...
# =========================================================
# FLASK APP & APPLICATION CONFIGURATION
# =========================================================

app = Flask(__name__)

# Session security / application settings.
# Keep the same secret key so existing sessions and current setup
# continue to work during development.
app.secret_key = "smart_lost_found_secret_key_2026"
...
def dashboard():
    ...
    try:
        ...
        lost_count = cursor.fetchone()

        lost_count = lost_count["total"] if lost_count else 0
        ...
        found_count = cursor.fetchone()

        found_count = found_count["total"] if found_count else 0
...
def search():
    ...
    try:
        ...
        if item_type.lower() in ["", "lost"]:
            lost_query = """
                SELECT l.id, l.item_name, l.category, l.description,
                       l.lost_location AS location, l.lost_date AS item_date,
                       l.color, l.identification_details, l.status,
                       'Lost' AS report_type, u.name AS reporter_name
                FROM lost_items l
            ...
            if keyword:
                lost_query += """
                    AND (l.item_name LIKE %s OR l.category LIKE %s
                    OR l.description LIKE %s
                    OR l.identification_details LIKE %s OR l.color LIKE %s)
                """
                v = "%" + keyword + "%"
                lost_params.extend([v, v, v, v, v])

            if category:
                lost_query += " AND LOWER(l.category) = LOWER(%s) "
                lost_params.append(category)

            if location:
                lost_query += " AND l.lost_location LIKE %s "
                lost_params.append("%" + location + "%")

            cursor.execute(lost_query, tuple(lost_params))
            lost_results = cursor.fetchall()
        else:
            lost_results = []

        if item_type.lower() in ["", "found"]:
            found_query = """
                SELECT f.id, f.item_name, f.category, f.description,
                       f.found_location AS location, f.found_date AS item_date,
                       f.color, f.identification_details, f.status,
                       'Found' AS report_type, u.name AS reporter_name
                FROM found_items f
            ...
            if keyword:
                found_query += """
                    AND (f.item_name LIKE %s OR f.category LIKE %s
                    OR f.description LIKE %s
                    OR f.identification_details LIKE %s OR f.color LIKE %s)
                """
                v = "%" + keyword + "%"
                found_params.extend([v, v, v, v, v])

            if category:
                found_query += " AND LOWER(f.category) = LOWER(%s) "
                found_params.append(category)

            if location:
                found_query += " AND f.found_location LIKE %s "
                found_params.append("%" + location + "%")

            cursor.execute(found_query, tuple(found_params))
            found_results = cursor.fetchall()
        else:
            found_results = []

        results = lost_results + found_results
        results.sort(key=lambda x: str(x.get("item_date") or ""), reverse=True)
        ...
        flash("Unable to search items. Please try again.", "error")
...
def item_details(item_type, item_id):
    ...
    try:
        ...
        if item_type == "lost":
            cursor.execute("""
                SELECT l.id, l.user_id, l.item_name, l.category,
                       l.description, l.lost_location AS location,
                       l.lost_date AS item_date, l.color,
                       l.identification_details, l.image, l.status,
                       l.created_at, u.name AS reporter_name,
            ...
            cursor.execute("""
                SELECT f.id, f.user_id, f.item_name, f.category,
                       f.description, f.found_location AS location,
                       f.found_date AS item_date, f.color,
                       f.identification_details, f.image, f.status,
                       f.created_at, u.name AS reporter_name,
                       u.email AS reporter_email
                FROM found_items f
                LEFT JOIN users u ON f.user_id = u.id
                WHERE f.id = %s
        ...
        print("ITEM DETAILS ERROR:", repr(e))
        flash("Unable to load item details.", "error")
...
def matches():
    ...
    try:
        ...
        # Current user's active lost items
        cursor.execute("""
            SELECT id, user_id, item_name, category, description,
                   lost_location, lost_date, color,
                   identification_details, status
            FROM lost_items
            WHERE user_id = %s
              AND status = 'Lost'
        ...
        cursor.execute("""
            SELECT id, user_id, item_name, category, description,
                   found_location, found_date, color,
                   identification_details, status
            FROM found_items
            WHERE status = 'Found'
            ORDER BY created_at DESC
        ...
        print("Matches:", len(matches_list))
        print("----------------------------------------")

    except Exception as e:
        print("SMART MATCH ERROR:", repr(e))
        if db:
            db.rollback()
        flash("Unable to load smart matches. Please try again.", "error")
...
def admin_dashboard():
    ...
    try:
        ...
        result = cursor.fetchone()
        total_lost = result["total"] if result else 0
        ...
        cursor.execute("""
            SELECT
                ...
                f.category AS found_category
            FROM claims c
            INNER JOIN users u ON c.claimant_id = u.id
            INNER JOIN lost_items l ON c.lost_item_id = l.id
            INNER JOIN found_items f ON c.found_item_id = f.id
            ORDER BY c.created_at DESC
            LIMIT 20
        """)
        ...
        flash("Unable to load admin dashboard.", "error")
...
def admin_claims():
    ...
    try:
        ...
        print("ADMIN CLAIMS ERROR:", repr(e))
        flash("Unable to load claims.", "error")
...
def admin_update_claim(claim_id):
    ...
    if not admin_note:
        if action == "approve":
            admin_note = "Claim approved by administrator."
        else:
            admin_note = "Claim rejected by administrator."
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, found_item_id, claimant_id, lost_item_id, status
            FROM claims
            WHERE id = %s
            LIMIT 1
        """, (claim_id,))
        ...
        if action == "approve":
            ...
            if not found_item:
                flash("Found item no longer exists.", "error")
                return redirect(url_for("admin_claims"))

            if found_item["status"] != "Found":
                flash("This found item is no longer available.", "error")
                return redirect(url_for("admin_claims"))
            ...
            cursor.execute("""
                ...
                SET
                    status = 'Rejected',
                    admin_note = 'This item was approved for another claim.'
                WHERE found_item_id = %s
                  AND id != %s
                  AND status = 'Pending'
        ...
        print("ADMIN CLAIM UPDATE ERROR:", repr(e))
        if db:
            db.rollback()
        flash("Unable to update claim.", "error")
        return redirect(url_for("admin_claims"))
...
def track():
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, item_name, category, lost_location AS location,
                   lost_date AS item_date, status, 'Lost' AS report_type
            FROM lost_items WHERE user_id = %s
            UNION ALL
            SELECT id, item_name, category, found_location AS location,
                   found_date AS item_date, status, 'Found' AS report_type
        ...
        cursor.execute("""
            SELECT c.id AS claim_id, c.claim_message, c.status AS claim_status,
                   c.admin_note, c.created_at,
        ...
        print("TRACK DATABASE ERROR:", repr(e))
        flash("Unable to load tracking information.", "error")
        return redirect(url_for("dashboard"))
    finally:
        if cursor:
            cursor.close()
    ...
    return render_template("track.html", reports=reports, claims=claims)
...
@app.route("/health")
def health():
    """Simple application/database health check."""
    db = None
    cursor = None
...
def page_not_found(error):
    """Handle unknown URLs without exposing internal details."""
    print("404 ERROR:", request.path)
    return render_template("404.html"), 404


@app.errorhandler(413)
def request_too_large(error):
    """Handle requests exceeding the configured size limit."""
    flash("The uploaded/request data is too large. Maximum size is 5 MB.", "error")
